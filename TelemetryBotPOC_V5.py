...
#conversion of embeddings
try:
    file_path = r"Dummy_dataset_V4.csv"       #replace the file_path for using in different source
...
def get_conversational_chain_telemetry(df):
    ...
    CASE 1:    
        ...
        1)Give me the telemetry for join a meeting via calendar 
        ...
        2)Provide me with the markers for join a meeting via calendar  in IOS and Android 
        SELECT [Platform],[subWorkLoadScenario],[Action_Type],[action_gesture],[module_name],[module_type],[action_outcome],[panel_type],[action_scenario],[action_scenario_type],[action_subWorkLoad],[thread_type],[databag.rsvp],[module_summary],[target_thread_type],[databag.meeting_rsvp],[databag.is_toggle_on],[databag.community_rsvp],[main_entity_type],[main_slot_app],[eventInfo.identifier],[databag.action_type],[subNav_entity_type] FROM df WHERE subWorkLoadScenario like "%user%asked%action%" AND Platform in ('IOS', 'Android') AND Action_Type = 'Primary'
        3)Could you give me the telemetry markers for join a meeting via calendar  in a meeting in Maglev
    ...
    CASE 2:
        ...
        1)what is the value for module name for join a meeting via calendar  in ios
        ...
        2)Provide me module type, thread type and action gesture for join a meeting via calendar  in IOS and Android 
        ...
        3)For join a meeting via calendar , what is the module summary, action outcome, panel type, action_scenario and action scenario type related to it
        ...
        4)action scenario, module type for join a meeting via calendar  for maglev, web and ios
        ...
        5)for join a meeting via calendar , what is thready type, action outcome, action subworkload and module summary
        ...
        6) give me panel type forjoin a meeting via calendar  in web and android
        ...
        7) action scenario forjoin a meeting via calendar 
...
def telemetry_response(user_question):
    try:
        ...
        if new_df.empty:
            return "Sorry, currently I do not have data for this action. Please try with a different question.. "
...
def get_conversational_chain_scenario(df):
    try:
        ...
        Example format:
            1. SELECT DISTINCT refer_index, platform, SubWorkLoadScenario FROM df_copy WHERE action_scenario = 'storenewmeeting';
            3. SELECT DISTINCT refer_index, platform, SubWorkLoadScenario FROM df_copy WHERE action_gesture = 'touch' AND module_name = 'savenewsessionbutton' AND module_type = 'pushbutton' AND action_outcome = 'send' AND platform = 'Android';
            
    
        Example:
        User Question: "Can you provide me the scenario where action scenario is store new meeting is used?"
        SQL Query should be like this: SELECT DISTINCT refer_index, platform, SubWorkLoadScenario FROM df_copy WHERE action_scenario = 'storenewmeeting';
        ...
        for line in text.splitlines(keepends=True):
            if "json" not in line and "nan" not in line:
                ...
                if line.startswith("Platform:"):
                    formatted_platform_line = f"## **{line.strip()}**"
                    inter_list.append(formatted_platform_line)
                    inter_list.append('\n')   
    ...
    try:
       ...
       **Instructions**: 
        Understand the user question and categorize the user question into one of the 4 categories -
            ...
            Example questions - 
                    Give me the telemetry markers for join a meeting via calendar 
            ...
            Example questions - 
                    Give me the recommended telemetry or marker for the new feature where it is inside the meeting feature?
