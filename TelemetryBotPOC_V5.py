...
1)what is the value for module name for joining a meeting through calendar in ios
    ...
    4)action scenario, module type for join a meeting
    SELECT [Platform],[subWorkLoadScenario],[Action_Type],[action_scenario],[module_type] FROM df WHERE subWorkLoadScenario like "%user%asked%action%" AND Action_Type = 'Primary'
    ...
    6) give me panel type for meeting leaving in web and ios
    SELECT [Platform],[subWorkLoadScenario],[Action_Type],[panel_type] FROM df WHERE subWorkLoadScenario like "%user%asked%action%" AND Action_Type = 'Primary' AND Platform in ('Web', 'IOS')
...
def get_conversational_chain_scenario(df):
    try:
        ...
        User Question: "action scenario is actiavte captions, what is the action related to it"
        SQL Query should be like this: SELECT refer_index, SubWorkLoadScenario,platform FROM df_copy WHERE action_scenario = 'actiavtecaptions' 
...
def get_Recommendation(df):
    try:
        ...
        ** Important ** : Here you need to give the recommendation of the new telemetry values based on those relevant actions in a dynamic Telemetry manner.
        ...
        --Community event - Leave a community event  by End for all / Leave community event for all   this is the category name you need to bucket where action or subworkloadscenario related to the list below.
     
     
        Community event - Leave a community event  by End for all
    ...
    **Second, if the user gives the prompt of the existing feature analysis and understands it, bucket the action in the respective category and go inside the respective category, and inside that bucket, see related actions or subworkload scenarios.
    and according to that, first understand the pattern of those existing features and give the new telemetries for the new feature the user needed.**
        ...
        "Recommend telemetry for the '**New Feature Name**' feature which is under the Reaction Menu"
       ...
       **Based on the user query refer the relevent platform and provide the  colums for that with the recommended values of you**
        ...
        **If the user's query pertains to a specific platform, respond with information relevant only to that platform**
        ...
        "Recommend telemetry for the '**New Feature Name**' feature which is under the Reaction Menu for Maglev platform"
    ...
    try:
        ...
        **Instructions**: 
        Understand the user question and categorize the user question into one of the 4 categories -
            1.1 Telemetry: If the user prompts only for retrieval of telemetry (or) telemetry markers, telemetry for a certain column for any actions or scenarios, categorize it into "Telemetry" category
            1.2 Telemetry: If user asks for specific column in a telemetry for a particular action, categorize that into "Telemetry" category
            Example questions - Give me the telemetry markers for join a meeting via calendar 
                                ...
                                what is the action gesture, module name and module type for sending angry reaction in web and ios
        ...
        - If asked for action scenario telemetry, it should be falling under category 1 as user needs telemetry of action scenario as output. If action scenario telemetry values is provided to you, in this case you need to give the action related to it. Don't confuse action with action scenario.
        - Action is the action that user is doing but action scenario is a telemetry column name
        ...
        **Output Format**:
        ...
        - **Expected Response**: 
            - Subworkloadscenario  
        - **User Query**: "action scenario type is schedule entry, what is the action related to it?"
        - **Expected Response**: 
            - Subworkloadscenario  
